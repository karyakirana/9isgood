<div>
    <x-molecules.modal title="Nomor Nota : {{isset($penjualan_data) ? $penjualan_data->kode : ''}}" size="xl" id="penjualan-retur-detail" wire:ignore.self>
        <form>
            <div class="row">
                <div class="col-6">
    ...
    @push('custom-scripts')
        <script>
            let modal_penjualan_retur_detail = document.getElementById('penjualan-retur-detail');
            let modalPenjualanReturDetail = new bootstrap.Modal(modal_penjualan_retur_detail);

            Livewire.on('hidePenjualanReturDetail', function (){
                modalPenjualanReturDetail.hide()
            })

            Livewire.on('showPenjualanReturDetail', function (){
                modalPenjualanReturDetail.show()
            })
        </script>
    @endpush
</div>
