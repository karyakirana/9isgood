<div>
    <x-molecules.modal title="Detail Stock Opname : {{isset($stock_data) ? $stock_data->kode : ''}}" size="xl" id="stock-detail" wire:ignore.self>
        <form>
            <div class="row">
                <div class="col-6">
                ...
                <div class="col-6">
                    <x-atoms.input.group-horizontal label="Jenis">
                        <x-atoms.input.plaintext>{{$stock_data->jenis ?? ''}}</x-atoms.input.plaintext>
                    </x-atoms.input.group-horizontal>
                </div>
            </div>
            <div class="row">
                <div class="col-6">
                    <x-atoms.input.group-horizontal label="Tgl Opname">
                        <x-atoms.input.plaintext>{{ isset($stock_data->tgl_opname) ? tanggalan_format($stock_data->tgl_opname) : ''}}</x-atoms.input.plaintext>
                    </x-atoms.input.group-horizontal>
                </div>
                <div class="col-6">
                    <x-atoms.input.group-horizontal label="Pegawai">
                        <x-atoms.input.plaintext>{{$stock_data->pegawai->nama ?? ''}}</x-atoms.input.plaintext>
                    </x-atoms.input.group-horizontal>
                </div>
            </div>
            <div class="row">
