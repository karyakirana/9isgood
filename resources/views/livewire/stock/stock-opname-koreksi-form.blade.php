                ...
                <form>
                    <div class="row">
                        <div class="col-6 mb-5">
                            <x-atoms.input.group-horizontal label="Tgl Input" name="tglInput">
                                <x-atoms.input.singledaterange wire:model.defer="tglInput" id="tglInput" readonly/>
                    ...
                    @forelse($dataDetail as $index => $row)
                        <tr>
                            ...
                            <x-atoms.table.td align="center">
                                ...
                                <x-atoms.button.btn-icon wire:click="removeLine({{$index}})"><i class="la la-trash fs-3"></i></x-atoms.button.btn-icon>
                            </x-atoms.table.td>
                        </tr>
                    @empty
                        <tr>
    ...
    @push('custom-scripts')
        <script>
            ...
            $('#tgl_nota').on('change', function (e) {
                let date = $(this).data("#tglInput");
                // eval(date).set('tglLahir', $('#tglLahir').val())
                console.log(e.target.value);
                @this.tglInput = e.target.value;
            })
        </script>
    @endpush
</div>
