<?php namespace App\Haramain\SistemHelper\Generator;
...
class GenPiutangPenjualanService
{
        ...
        try {
            ...
            foreach ($penjualanAll as $penjualan){
                ...
                if ($piutangPenjualan){
                    ...
                    $this->saldoPiutangPenjualanRepo->penjualanRollback($penjualan->customer_id, $penjualan->total_bayar);
                    // update
                ...
                $this->saldoPiutangPenjualanRepo->penjualan($penjualan->customer_id, $penjualan->total_bayar);
            }
            \DB::commit();
            return [
                'status'=>true,
    ...
    public function handleGeneratePenjualanRetur()
    {
        ...
        try {
            ...
            foreach ($penjualanReturAll as $penjualanRetur) {
                ...
                if ($piutangPenjualan){
                    // rollback saldo piutang penjualan repo
                    $this->saldoPiutangPenjualanRepo->returRollback($penjualanRetur->customer_id, $piutangPenjualan->total_bayar);
                    // update
                    $piutangPenjualan->update([
                ...
                // update saldo piutang penjualan
                $this->saldoPiutangPenjualanRepo->retur($penjualanRetur->customer_id, $penjualanRetur->total_bayar);
            }
            \DB::commit();
            return [
                'status'=>true,
